...
class Upload{
    ...
    public function upload($files='') {
        ...
        $upType = C('FILE_UPLOAD_TYPE');

        //modified by tiancheng
        if ('local' == $upType && !empty($info) && !isset($info['fileUrl'])) {
            $name = $info['file']['savename'];
            $path = $info['file']['savepath'];
            $info['fileUrl'] = C('HUANJU_DOMAIN') . 'Uploads/' . $path . $name;
        } else if ('qiniu' == $upType || 'ks3' == $upType) {
            $info['fileUrl'] = $info['file']['url'];
        }
        if(isset($finfo)){
            finfo_close($finfo);
        }
        return empty($info['fileUrl']) ? false : $info;
    }
    ...
    public function uploadToStorage($config) {
        return $this->uploader->upload($config);
    }
}
